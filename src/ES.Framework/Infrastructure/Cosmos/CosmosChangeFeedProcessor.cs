...
/// <summary>Class CosmosChangeFeedProcessor. Implements the <see cref="IHostedService" /></summary>
...
public class CosmosChangeFeedProcessor : IHostedService
{
	 ...
	 /// <summary>Initializes a new instance of the <see cref="CosmosChangeFeedProcessor" /> class.</summary>
	 ...
	 private async Task OnChangesDelegate(ChangeFeedProcessorContext context,
		 IReadOnlyCollection<EventDocument> documents, CancellationToken cancellationToken) {
		  foreach(var document in documents) {
				_logger.LogInformation("Processing event {1}...", document.PayloadType);
