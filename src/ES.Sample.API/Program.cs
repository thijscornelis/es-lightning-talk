...
builder.Services.AddTransient<IPartitionKeyResolver, PartitionKeyResolver>();
...
builder.Services.Configure<JsonOptions>(o => {
	...
	foreach(var converter in options.Converters) o.JsonSerializerOptions.Converters.Add(converter);
});
...
app.MapGet("api/bank-accounts/{id}",
	async ([FromRoute] Guid id, IBankAccountRepository repository, CancellationToken cancellationToken) => {
		...
		return aggregate == null
			? Results.NotFound()
			: Results.Json(aggregate.State, statusCode: (int) HttpStatusCode.OK,
...
void RegisterRepositories(WebApplicationBuilder webApplicationBuilder) {
	...
	webApplicationBuilder.Services.AddScoped<IBankAccountRepository, BankAccountRepository>(c => {
		...
		return new BankAccountRepository(new CosmosDocumentRepository(container, c.GetService<ICosmosQuery>()),
			c.GetService<IEventDocumentConverter>(),
			c.GetService<IPartitionKeyResolver>());
	});
}
